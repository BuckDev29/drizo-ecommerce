...
## Despliegue Recomendado (Gratis o Costo Bajo)

### Opcion A (practica para portafolio)

- Frontend: Vercel
- Backend: Render o Railway
- Base de datos: Railway MySQL, Neon + proxy, o un MySQL administrado

Configura en frontend `NEXT_PUBLIC_API_URL` apuntando al backend desplegado.

### Opcion B (todo en un solo proveedor)

- Full stack en Railway/Render con servicios separados de frontend y backend

## Checklist para Subir a GitHub

- Confirmar que `backend/.env` y `frontend-next/.env.local` no se suben
- Usar solo `.env.example` con placeholders
- Revisar historial si en el pasado se subio algun secreto
- Rotar credenciales si alguna vez estuvieron en remoto
- Hacer commit con mensaje claro

## Estado del Proyecto

Proyecto funcional y apto para portafolio full-stack.

Siguiente mejora sugerida: agregar pruebas (unit/integration) y `database/schema.sql` completo para onboarding en un solo comando.