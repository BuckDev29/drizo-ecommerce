...
exports.addToCart = async (req, res) => {
  try {
    ...
    if (existing.length > 0) {
      ...
      return res.status(201).json({ message: "Producto agregado al carrito", id: result.insertId });
    }
  } catch (error) {
...
exports.updateCartItem = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) return res.status(404).json({ message: "Item no encontrado en carrito" });
